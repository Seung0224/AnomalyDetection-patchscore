...
from inference import anomaly_detect
...
class App(ctk.CTk):
    ...
    def _infer_thread(self, img_path, image_snapshot, save_dir, save_origin, save_overlay):
        ...
        try:
            overlay_img = anomaly_detect(img=image_snapshot, model=self.model, device=self.device)
            err = None
        except Exception as e:
            overlay_img = None
            err = str(e)
    ...
    def _set_proc_time(self, ms):
        ...
        if self.lbl_proc is None: return
        self.lbl_proc.configure(text="Processing Time: N/A" if ms is None else f"Processing Time: {ms:.1f} ms")
