...
from InferOverlay import _predict, _make_overlay, draw_status_frame
...
def anomaly_detect(img: ImgLike, model, device: str, **kwargs,):
    ...
    # 새 옵션
    threshold = float(kwargs.pop("threshold", 2.0))
    index     = kwargs.pop("index", None)   # 1-based
    ...
    # Predict → amap
    amap, score_mem = _predict(model, x_t)   # ← 변경: tuple 반환
    score = float(score_mem) if score_mem is not None else float(amap.max())
    ...
    # 테두리+라벨 렌더링
    out = draw_status_frame(base, status=status, score=score, index=index, total=total)
    return out, score
